...
app = Flask(__name__)
api = Api(app)
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///database5.db'
db = SQLAlchemy(app)
ma = Marshmallow(app)
county =0
...
class order3(Resource):
    ...
    def put(self):    
        ...
        if (county ==0) :
            response = requests.patch("http://192.168.1.50:6000/" + "book2", {"item_id" : args['item_id'] })
            county=1
        elif (county ==1):
             response = requests.patch("http://192.168.1.50:7000/" + "book2", {"item_id" : args['item_id'] })
             county=2
        else :

             response = requests.patch("http://192.168.1.50:9000/" + "book2", {"item_id" : args['item_id'] })
             county =0
        ...
        if(temppp['exsist']==-1):
            video=order(order_id=-1,book_id=args['item_id'], date=str(datetime.datetime.utcnow()))
            return video
        elif(temppp['exsist']==-2):
            video=order(order_id=-2,book_id=args['item_id'], date=str(datetime.datetime.utcnow()))
            return video
        else: 
