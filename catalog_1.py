...
class catalog7(Resource):
	def patch(self ):
		...
		if not result:
			...
			response = requests.put("http://192.168.1.50:7000/" + "follow_up_buy", {"item_id" :args['item_id']  ,"number_in_stock" : result.number_in_stock})
...
class catalog3(Resource):
	...
	def put(self, book_id):
		...
		response = requests.put("http://192.168.1.50:7000/" + "follow_up_create", {"item_id" :book_id , "topic" : args['topic'],"number_in_stock":args['number_in_stock'],"cost":args['cost'], "titel":args['titel']})
...
class front969696(Resource):
	def post(self, id):
		...
		args = catalog_put_args.parse_args()
		result = catalog.query.filter_by(item_id = id).first()
...
api.add_resource(front969696, "/follow_up_update222221/<int:id>")  




class front96969697(Resource):
	def put(self):
		catalog_put_args = reqparse.RequestParser()
		catalog_put_args.add_argument("number_in_stock", type=int, help="#stock not avalible", required=True)
...
api.add_resource(front96969697, "/follow_up_buy")  


@app.route('/follow_update_book_cost_and_number_in_stock/<id>', methods=['POST'])
...
@app.route('/follow_new_book2222/<id>', methods=['put'])
def add_product2(id):
...
class catalog4(Resource):
	...
	def post(self, video_id):
		...
		response = requests.put("http://192.168.1.50:7000/" + "follow_up_update", {"item_id" :video_id ,"topic" :args['topic'] , "cost" :args['cost'] ,"number_in_stock" :args['number_in_stock']})
...
def update_product4(id):
	...
	response = requests.put("http://192.168.1.50:7000/" + "follow_up_cost", {"item_id" :id , "number_in_stock" : product.number_in_stock ,"cost" :product.cost })
...
def update_product5():
	...
	response = requests.post("http://192.168.1.50:7000/" + "follow_decraese_in_stock", {"item_id" :args['item_id']} )
...
api.add_resource(frontaida, "/follow_book_decraese_in_stock")


class frontaida2(Resource):
	def post(self):
		catalog89_update_args = reqparse.RequestParser()
		catalog89_update_args.add_argument("item_id", type=str, help="#id")
		args = catalog89_update_args.parse_args()
...
api.add_resource(frontaida2, "/follow_book_incraese_in_stock")  
...
api.add_resource(frontaida3, "/folloe_delete/<id>")    

@app.route('/book_incraese_in_stock', methods=['POST'])

def update_product67():
	catalog89_update_args = reqparse.RequestParser()
	catalog89_update_args.add_argument("item_id", type=str, help="#id")
	args = catalog89_update_args.parse_args()
	product = catalog.query.get(args['item_id'])
	...
	db.session.commit()
	response = requests.put("http://192.168.1.50:8000/" + "consistant", {"item_id" :args['item_id'] , "topic" : product.topic})
	response = requests.post("http://192.168.1.50:7000/" + "follow_incraese_in_stock", {"item_id" :args['item_id'] })
...
def delete_product8(id):
  ...
  response = requests.post("http://192.168.1.50:7000/" + "follow_delete_book/" + str(id))
...
def add_product10(id):
  ...
  db.session.add(new_product)
  db.session.commit()
  response = requests.put("http://192.168.1.50:8000/" + "consistant", {"item_id" :id , "topic" : args['topic'] })
  response = requests.post("http://192.168.1.50:7000/" + "follow_new_book/" + id  , {"topic" : args['topic'] , "cost" : args['cost'] ,"number_in_stock" : args ['number_in_stock'] , "titel" : args['titel']})
