...
class front_end1(Resource):
    ...
    def get(self,book_topic):
        ...
        if(val is None or  len(val)<=1 ):
            if (count == 0) :
                response = requests.get("http://192.168.1.50:6000/" + "search", {"topic" :book_topic})
                count=1
            elif (count ==1 ):
                response = requests.get("http://192.168.1.50:7000/" + "search", {"topic" :book_topic}) 
                count=2  
            else :
                response = requests.get("http://192.168.1.50:9000/" + "search", {"topic" :book_topic})
                count=0
...
def get2(item_id):
    ...
    if (val is None or  len(val)<=1 ):
        if count2 == 0 :
            response = requests.get("http://192.168.1.50:6000/" + "lookup", {"item_id" :item_id})
            count2=1
        elif count2 == 1:
            response = requests.get("http://192.168.1.50:7000/" + "lookup", {"item_id" :item_id})    
            count2=2
        else:
            response = requests.get("http://192.168.1.50:9000/" + "lookup", {"item_id" :item_id})
            count2=0
...
def buy(item_id):
    ...
    global count3
    if count3 == 0 :
        response = requests.put("http://192.168.1.50:3000/" + "buy", {"item_id" :item_id})
        count3=1
    elif count3 == 1:
        response = requests.put("http://192.168.1.50:4000/" + "buy", {"item_id" :item_id})
        count3=2
    else:
        response = requests.put("http://192.168.1.50:5000/" + "buy", {"item_id" :item_id})
        count3=0
    ...
    if temppp['order_id'] ==-1 :
        ...
        if(val is None or  len(val)<=1 ):
            r.set(name =hashkey2 , value = json.dumps(data))
